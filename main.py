...
def hit():
    ...
    print(f"Your cards: {user_cards}, current score: {user_count}")
    print(f"Computer's first card: {dealer_cards[0]}")

def stand():
    user_count = sum(user_cards)
    dealer_count = sum(dealer_cards)
    print (f"Your final hand {user_cards}, final count: {user_count}")
    print(f"Computer's final hand {dealer_cards}, final count: {dealer_count}")
    if user_count < 22:
...
while should_continue:
    if user_count < 21:
        go_again = input("Type 'y' to get another card, type 'n' to pass: ")
        if go_again == 'y':
            hit()
            user_count = sum(user_cards)
        elif go_again == 'n':
